// functions/marketDiagnostics.js
// Admin-only diagnostics: tests Bridge endpoints with dummy data

import { createClientFromRequest } from 'npm:@base44/sdk@0.8.6';
import { callBridge } from './_shared/bridgeClient.js';

const BUILD_SIGNATURE = 'market-diagnostics-v1-20260107';

Deno.serve(async (req) => {
  const correlationId = `market-diag-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
  ...
  try {
    ...
    console.log(`[marketDiagnostics:${correlationId}] COMPLETE status=${report.overall_status}`);
    ...
    console.error(`[marketDiagnostics:${correlationId}] CRITICAL_ERROR: ${error.message}`);
