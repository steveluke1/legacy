...
Deno.serve(async (req) => {
  ...
  try {
    console.log(`[supabase_debug:${requestId}] START`);
    ...
    console.log(`[supabase_debug:${requestId}] host=${host}`);
    ...
    if (e1) {
      console.error(`[supabase_debug:${requestId}] auth_users read failed: ${e1.message}`);
      return Response.json({
        ok: false,
        error: {
    ...
    if (e2) {
      console.error(`[supabase_debug:${requestId}] auth_sessions read failed: ${e2.message}`);
      return Response.json({
        ok: false,
        error: {
          code: 'SUPABASE_DEBUG_FAILED',
    ...
    if (e3) {
      console.error(`[supabase_debug:${requestId}] auth_audit_log read failed: ${e3.message}`);
      return Response.json({
        ok: false,
        error: {
          code: 'SUPABASE_DEBUG_FAILED',
    ...
    console.log(`[supabase_debug:${requestId}] BEFORE counts: users=${authUsersCountBefore} sessions=${authSessionsCountBefore} audit=${authAuditLogCountBefore}`);
    ...
    if (e4) {
      console.error(`[supabase_debug:${requestId}] INSERT failed: ${e4.message}`);
      return Response.json({
        ok: false,
        error: {
          code: 'SUPABASE_DEBUG_FAILED',
    ...
    console.log(`[supabase_debug:${requestId}] INSERT success: audit_log_id=${insertedRow?.id || 'unknown'}`);
    ...
    if (e5) {
      console.error(`[supabase_debug:${requestId}] auth_audit_log read after failed: ${e5.message}`);
      return Response.json({
        ok: false,
        error: {
          code: 'SUPABASE_DEBUG_FAILED',
    ...
    console.log(`[supabase_debug:${requestId}] AFTER count: audit=${authAuditLogCountAfter}`);
    console.log(`[supabase_debug:${requestId}] SUCCESS - delta=${authAuditLogCountAfter - authAuditLogCountBefore}`);
    ...
    console.error(`[supabase_debug:${requestId}] FATAL:`, error);
    return Response.json({
      ok: false,
      error: {
        code: 'SUPABASE_DEBUG_FAILED',
