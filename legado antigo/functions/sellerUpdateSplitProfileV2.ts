// functions/sellerUpdateSplitProfileV2.js
// Update seller's Efí Split configuration (V2 - bypass stuck deployment)
// VERSION: 1.0.0 - BODY TOKEN AUTH (custom JWT HS256)

const BUILD_STAMP = "sellerUpdateSplitProfileV2-v1-bodytoken-20260113-deployed";
...
Deno.serve(async (req) => {
  try {
    ...
    const correlationId = `seller-split-v2-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
    ...
    if (!jwtSecret) {
      console.error(`[sellerUpdateSplitProfileV2:${correlationId}] JWT_SECRET missing`);
      return Response.json({
        ok: false,
    ...
    try {
      ...
      console.log(`[sellerUpdateSplitProfileV2:${correlationId}] JWT_INVALID reason=${e.message}`);
    ...
    // Check if profile exists (ownerField = user_id per schema)
    ...
    console.log(`[sellerUpdateSplitProfileV2:${correlationId}] SUCCESS user=${userId}`);
    ...
    console.error('[sellerUpdateSplitProfileV2] ERROR:', error);
    return Response.json({
      ok: false,
      error: {
        code: 'INTERNAL_ERROR',
