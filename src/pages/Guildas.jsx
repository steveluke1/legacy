...
import { Shield, Users, Trophy, Swords, Search, Filter, ChevronRight } from 'lucide-react';
...
const mockGuilds = [
  { id: '1', name: 'Apocalypse', slug: 'apocalypse', faction: 'Procyon', level: 12, member_count: 98, recruiting: true, description: 'A guilda mais temida de Nevareth' },
  { id: '2', name: 'Phoenix', slug: 'phoenix', faction: 'Capella', level: 11, member_count: 95, recruiting: false, description: 'Das cinzas, renascemos mais fortes' },
  { id: '3', name: 'Legends', slug: 'legends', faction: 'Procyon', level: 10, member_count: 87, recruiting: true, description: 'Lendas nunca morrem' },
  { id: '4', name: 'Elite', slug: 'elite', faction: 'Capella', level: 9, member_count: 82, recruiting: true, description: 'Somente os melhores' },
  { id: '5', name: 'Warriors', slug: 'warriors', faction: 'Procyon', level: 8, member_count: 76, recruiting: false, description: 'Guerreiros de coração' },
  { id: '6', name: 'Shadow Legion', slug: 'shadow-legion', faction: 'Capella', level: 7, member_count: 71, recruiting: true, description: 'Nas sombras, conquistamos' },
];
...
export default function Guildas() {
  ...
  const loadGuilds = async () => {
    setLoading(true);
    try {
      const data = await base44.entities.Guild.list('-level');
      if (data.length > 0) {
        setGuilds(data);
      } else {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredGuilds.map((guild, index) => (
            ...
            >
              <Link to={createPageUrl(`GuildaDetalhe?slug=${guild.slug}`)}>
                <GlowCard className="p-6 h-full group" glowColor={guild.faction === 'Capella' ? '#4A90D9' : '#D94A4A'}>
                  ...
                  <div className="mt-4 flex items-center justify-end text-[#19E0FF] text-sm group-hover:translate-x-1 transition-transform">
